...
formatted_battery_data = re.search(r"\s(\d+)\%\;.(.+)\;.(.+).present", raw_battery_data)
...
if (
    ...
    and int(percentage) < 60
    and battery_status == "discharging"
) or (int(percentage) <= 30 and battery_status == "discharging"):
    embed = DiscordEmbed(title="Mac Battery Low")

    embed.add_embed_field(name="Data", value=f"Percentage:\t{percentage}", inline=False)
